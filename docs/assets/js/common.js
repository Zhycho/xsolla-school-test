...
document.addEventListener('DOMContentLoaded', function() {
    let eventsList = document.querySelector('.js--events-list');
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      card.setAttribute('data-city', events[i].city);
      img.src = events[i].image;
      img.alt = events[i].name;
      time.setAttribute('datetime', events[i].date);
      time.textContent = events[i].date.slice(0, 2);
      time.classList.add('card__label');
      h2.textContent = events[i].name;
    ...
    let elementsOfEventsList = document.querySelectorAll('.js--events-list li');
    let citySelect = document.querySelector('select[name="cities"]');
    // let monthsSelect = document.querySelector('select[name="months"]');
    
    citySelect.addEventListener('change', function() {
      let value = this[this.selectedIndex].value;
      
      if (value !== 'All') {
        elementsOfEventsList.forEach(element => {
          element.dataset.city === value ? element.style.display = '' : element.style.display = 'none';
        });
      } else {
        elementsOfEventsList.forEach(element => element.style.display = '');
      }
    });
    ...
    //     if (value !== 'All') {
    //       elementsOfEventsList.forEach(element => {
    //         +time.getAttribute('datetime').slice(3, 6) == +value ? element.style.display = '' : element.style.display = 'none';
    //       });
    //     } else {
    //       elementsOfEventsList.forEach(element => element.style.display = '');
    //     }
